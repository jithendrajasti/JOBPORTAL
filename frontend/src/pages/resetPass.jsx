...
const ResetPass = () => {
    ...
    const firstHandler = async (e) => {
        ...
        try {
            ...
            const { data } = await axios.post(backendUrl + '/api/company/reset-pass', { email: trimmedEmail })
            if (data.success) {
    ...
    return (
        <div className='flex items-center justify-center min-h-screen px-6 sm:px-0'>
            <img
                src={assets.logo}
                alt=''
                className='absolute left-5 sm:left-20 top-5 w-36 sm:w-52 cursor-pointer'
                onClick={() => navigate('/')}
            />
